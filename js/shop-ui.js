...
function updateCartUI() {
  ...
  cart.forEach(c => {
    ...
    if (!p) return;
    const subtotal = (p.price ?? 0) * c.qty;
    sum += subtotal;
...
export function refreshProducts(shopifyProducts) {
  ...
  // Shopify商品とローカル商品を商品名で照合し、価格とvariantIdを更新
  for (const sp of shopifyProducts) {
    ...
    if (local && local.status === 'on-sale') {
      ...
      if (sp.images && sp.images.length > 0 && sp.images[0].src) {
        // ローカル画像を優先するためコメントアウト
        // local.img = sp.images[0].src;
      }
    }
  }
